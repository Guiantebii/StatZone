  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        <label
          htmlFor={`formacao-${timeId}`}
          className="text-[10px] uppercase tracking-wider text-slate-500 font-semibold mb-0.5 block"
        >
        ...
        <div className="flex gap-2 items-end">
              ...
              >
                ...
                {posicoesDisponiveis.map((p) => (
                  <option key={p} value={p}>
                    {POSICAO_ABBR[p] || p.replace(/_/g, ' ')} — {p.replace(/_/g, ' ')}
                  </option>
                ))}
              </select>
            </div>
          ...
          <div>
            ...
            <select
              ...
              value={funcao}
              onChange={(e) => {
                setFuncao(e.target.value as FuncaoEscalacao);
                setSelectedPosicao('');
              }}
      ...
      {titulares.length > 0 && (
        <div>
          ...
          <div className="space-y-1">
            {titulares.map((e) => (
              <div
                key={e.id || e.jogadorId}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-white/[0.02] text-xs text-slate-300"
              >
    ...
    try {
      const [escRes, jogMandRes, jogVisRes] = await Promise.all([
        api
          ...
          .catch(() => ({ data: { partidaId, titulares: [], reservas: [] } })),
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="glass-strong rounded-2xl w-full max-w-3xl mx-4 border border-white/[0.06] shadow-2xl animate-scale-in max-h-[90vh] flex flex-col">
        ...
        <div className="flex justify-end gap-3 px-6 py-4 border-t border-white/[0.06] shrink-0">
          ...
          <Button onClick={onSaved} disabled={saving || loading}>
            Concluído
          </Button>
        </div>
      </div>

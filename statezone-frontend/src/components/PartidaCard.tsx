  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-3">
        ...
        <span className={`inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider ${statusInfo.className}`}>
          {statusInfo.label}
        </span>
      </div>
      ...
      <div className="flex items-center gap-3 py-2">
        <div className="flex-1 flex items-center justify-end gap-2 text-right">
          <Link to={`/times/${partida.timeMandanteId}`} onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <span className={`text-sm font-semibold ${showScore && partida.golsMandante > partida.golsVisitante ? 'text-accent' : 'text-slate-200'}`}>
        ...
        <div className="flex-shrink-0 w-16 text-center">
          {showScore ? (
            ...
            <div className="flex flex-col items-center gap-0.5">
              ...
              <span className="text-[10px] text-slate-500 font-medium leading-tight">{formatDate(partida.dataPartida)}</span>
        ...
        <div className="flex-1 flex items-center gap-2">
          <Link to={`/times/${partida.timeVisitanteId}`} onClick={(e) => e.stopPropagation()} className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <img
              ...
              className="w-8 h-8 rounded-full bg-white/5"
            />
            <span className={`text-sm font-semibold ${showScore && partida.golsVisitante > partida.golsMandante ? 'text-accent' : 'text-slate-200'}`}>

...
export default function JogadorForm({ jogador, onClose, onSaved }: JogadorFormProps) {
  ...
  const [altura, setAltura] = useState<string | number>(jogador ? (jogador.altura ?? '') : 1.8);
  ...
  return (
    <Modal title={jogador ? 'Editar jogador' : 'Novo jogador'} onClose={onClose}>
      <p className="text-xs text-slate-500 mb-4">
        {jogador ? `Editando "${jogador.nome}"` : 'Preencha os dados do jogador'}
      </p>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              >
                ...
                {times.map((t) => (
                  <option key={t.id} value={t.id} className="bg-[#0d1f3c]">
                    {t.nome}
                  </option>
                ))}
              </select>
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className={inputClass}
                value={numeroCamisa}
                onChange={(e) => setNumeroCamisa(Number(e.target.value))}
                min={1}
                max={99}
              />
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                ...
                className={inputClass + ' appearance-none'}
                value={peForte}
                onChange={(e) => setPeForte(e.target.value)}
              >
